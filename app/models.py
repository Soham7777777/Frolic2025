...
class ConstrainedImagePathModel(CustomPydanticBase):
    ...
    def validate_model(self) -> Self:
        ...
        if self.path.stat().st_size > self.max_bytes:
            raise ValueError(f'The file size must be less than ~{str(round(self.max_bytes/1000, 1)) + "kb" if self.max_bytes >= 1000 else str(self.max_bytes) + " bytes"}.')
...
class UserWiseProfilePicture(db.Model): # type: ignore
    ...
    path: Mapped[str] = mapped_column(ConstrainedImagePath(max_bytes=5*KB), unique=True, init=True)
...
class User(db.Model, TimestampMixin): # type: ignore
    ...
    role: Mapped[Role] = mapped_column(init=True)
...
class Event(db.Model, TimestampMixin): # type: ignore
    ...
    created_by: Mapped[User] = relationship(init=True)
    modified_by: Mapped[User] = relationship(init=False)
...
class Team(db.Model): # type: ignore
    ...
    leader_id: Mapped[int] = mapped_column(ForeignKey('participant.participant_id'), init=False, unique=True)
    evnet_id: Mapped[int] = mapped_column(ForeignKey('event.event_id'), init=False, unique=True)
